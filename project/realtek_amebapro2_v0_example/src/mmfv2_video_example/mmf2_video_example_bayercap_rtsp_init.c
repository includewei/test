...
#if V1_RAW_RESOLUTION == VIDEO_VGA
#define V1_RAW_WIDTH	640
#define V1_RAW_HEIGHT	480
#elif V1_RAW_RESOLUTION == VIDEO_HD
#define V1_RAW_WIDTH	1280
#define V1_RAW_HEIGHT	720
#elif V1_RAW_RESOLUTION == VIDEO_FHD

#if USE_SENSOR == SENSOR_GC4653
#define V1_RAW_WIDTH	2560
#define V1_RAW_HEIGHT	1440
#else
#if USE_SENSOR == SENSOR_JXF51
#define V1_RAW_WIDTH	1536
#define V1_RAW_HEIGHT	1536
#else
#define V1_RAW_WIDTH	1920
#define V1_RAW_HEIGHT	1080
#endif
#endif
...
static video_params_t video_bayercap_params = {
	...
	.type = V1_RAW_TYPE,
	.resolution = V1_RAW_RESOLUTION,
	.width = V1_RAW_WIDTH,
	.height = V1_RAW_HEIGHT,
	.fps = V1_RAW_FPS,
	.gop = V1_RAW_GOP,
	.bps = V1_RAW_BPS,
...
void file_save(char *file_path, uint32_t data_addr, uint32_t data_size)
{
	...
	if (f_open(&m_file1, savefilepath, FA_OPEN_ALWAYS | FA_READ | FA_WRITE) == FR_OK) {
		if (data_size > 0) {
			...
			f_write(&m_file1, (void *)data_addr, data_size, NULL);
			f_close(&m_file1);
		}
	}
...
void mmf2_video_example_bayercap_rtsp_init(void)
{
	...
	int voe_heap_size = video_voe_presetting(1, RTSP_WIDTH, RTSP_HEIGHT, RTSP_BPS, 0,
						1, V1_RAW_WIDTH, V1_RAW_HEIGHT, V1_RAW_BPS, 0,
						0, 0, 0, 0, 0,
						0, 0, 0);

...
int nand_pfw_update_ptable(void);
...
int nor_pfw_update_flash(uint8_t *nor_addr, uint8_t *data, int size)
{
	...
	if (size_a) {
		...
		// erase sector
		flash_erase_sector(&nor_flash, (uint32_t)addr_cpy);
		// write sector
...
int nor_pfw_write_mani(void *fr, void *mani)
{
	...
	// write sector
	flash_stream_write(&nor_flash, (uint32_t)mani_nor, 4096, mani);
...
int nor_pfw_write_mani_unpt(void *fr, void *data, int size)
{
	...
	flash_erase_sector(&nor_flash, (uint32_t)mani_nor);
	// write sector
	flash_stream_write(&nor_flash, (uint32_t)mani_nor, 4096, tmp);

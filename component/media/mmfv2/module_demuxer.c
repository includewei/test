...
Reaction:
		...
		case DEMUXER_END:
			...
			if (ctx->demuxer_v_state == DEMUXER_STOP) {
				if (xTaskGetTickCount() > ((ctx->real_audio_timestamp + ctx->audio_duration) / (ctx->mp4_demuxer_ctx->audio_sample_rate / 1000))) {
					ctx->start_timestamp = xTaskGetTickCount();
					ctx->cur_a_index = 0;
...
Reaction:
		...
		case DEMUXER_PAUSE: {
			...
			mm_context_t *mctx = (mm_context_t *)ctx->parent;
			mm_queue_item_t *output_item;
			//mm_printf(" xTaskGetTickCount() = %d, %d ", xTaskGetTickCount(), (ctx->real_video_timestamp + ctx->video_duration) / 90);
			if (xTaskGetTickCount() > ctx->real_video_timestamp + (ctx->video_duration / 90)) {
				//send the same frame

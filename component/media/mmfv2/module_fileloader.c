...
static void SD_file_loader_init(void *);
static void SD_file_loader_deinit(void *);
static int SD_file_load_file(void *p, uint32_t *pFrame, uint32_t *pSize, char *frameFilePath);
	...
	while (load_file_count < ctx->load_file_num) {
		...
		if (is_output_ready) {
			...
			*}*/
			...
			if ((uint32_t *)test_data_addr != NULL) {
				free((uint32_t *)test_data_addr);
			}
		}
	}
	printf("\r\nFiles loading done...\r\n");
...
static int SD_file_load_file(void *p, uint32_t *pFrame, uint32_t *pSize, char *frameFilePath)
{
	...
	fread(file_buf, file_size, 1, m_file);
	fclose(m_file);
...
static int tftp_file_load_file(void *p, uint32_t *pFrame, uint32_t *pSize, char *frameFilePath)
{
...
static int ftp_file_load_file(void *p, uint32_t *pFrame, uint32_t *pSize, char *frameFilePath)
{
	media_ctx_t *mactx = (media_ctx_t *)p;
	...
	int len = 0;
	int ret = client->ftpClientGetBuf(file_buf, &len, frameFilePath, FTP_CLIENT_BINARY, ftpClientNetBuf);

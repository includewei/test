...
/*********************************************************************************************
// \brief:	   import point from binary
...
int rtw_crypto_ecc_point_read_binary(sae_ecc_crypto *ecc, u8 *buf, size_t buf_len, sae_ecc_point *p)
{
	...
	prime_len = mbedtls_mpi_size(&ecc->P);
	pbuf_len = 2 * prime_len + 1;
	if ((buf_len + 1) != pbuf_len) {
		ret = -1;
		goto cleanup;
	}

	pbuf = rtw_zmalloc(pbuf_len);
	if (pbuf == NULL) {
		ret = -1;
		goto cleanup;
	}
	...
	pbuf[0] = 0x04;
	rtw_memcpy(pbuf + 1, buf, pbuf_len - 1);

	ret = mbedtls_ecp_point_read_binary(ecc, p, pbuf, pbuf_len);

cleanup:
	if (pbuf) {
		rtw_mfree(pbuf, pbuf_len);
	}
	return ret;
}

/*********************************************************************************************
// \brief:	   export point to binary

// \param p:       export point of ecc
// \param buf_len: output buffer size
// \param x:       output buffer x
// \param y:       output buffer y
//
return:    0 if successful
		   -1 if failed
*********************************************************************************************/
int rtw_crypto_ecc_point_write_binary(sae_ecc_point *p, unsigned int buf_len, u8 *x, u8 *y)
{
	...
	if (y) {
		if (rtw_crypto_bignum_write_binary(&p->Y, y, buf_len) < 0) {
			return -1;
		}
	}
...
//
// \param X		    pointer of big number to use
// \param z		    The integer value to set
//
// \return			0 if successful
//        			-1 if failed
//
//***************************************************************************************************
int rtw_crypto_bignum_lset(sae_crypto_bignum *X, int z)
{
	int ret = 0;
	MBEDTLS_MPI_CHK(mbedtls_mpi_lset(X, z));

...
int MP3PCMUpSample3X(short *outbuf, short *inbuf, int insize, short *last_end);
#endif
void MP3GetLastFrameInfo(HMP3Decoder hMP3Decoder, MP3FrameInfo *mp3FrameInfo);
int MP3GetNextFrameInfo(HMP3Decoder hMP3Decoder, MP3FrameInfo *mp3FrameInfo, unsigned char *buf);
int MP3FindSyncWord(unsigned char *buf, int nBytes);
#if defined(CONFIG_PLATFORM_8195BHP) || defined(CONFIG_PLATFORM_8735B)
void resample_44_1k_stereo_to_16k_mono(short* buf_in, int samples_in, short* buf_out, int *samples_out, int channel_sel, int is_decimate);
void MP3DecodeSetChannel(int channel);
#endif

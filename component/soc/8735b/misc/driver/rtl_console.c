...
static void _put_char(char c)
{
	...
	_fflush_r(_REENT, stdout);
}

static void _send_string(char *str)
{
	unsigned int i = 0;
	while (str[i] != '\0') {
...
static void _send_buf(u8 *buf, u32 len)
{
	char *st_p = (char *)buf;
	if (!len || (!buf)) {
		return;
...
void default_key_handling(char rc)
{
	...
	if (rc == KEY_ESC) {
		...
		if (buf_count > 0) {
			...
			_send_string((char *)STR_END_OF_MP_FORMAT);
		}
	} else if (rc == KEY_BS) {
		if (buf_count > 0) {
			buf_count--;
...
{
	uart_send_string((char *)("key: ctrl c\n\r"));
}

...
static void nn_save_handler(char *file_name, uint32_t data_addr, uint32_t data_size)
{
	...
	pre_tensor_out = (vipnn_out_buf_t *)data_addr;
	...
	int ret = media_save_file(nn_fn, (char *)pre_tensor_out->vipnn_res.frec_res.result, sizeof(pre_tensor_out->vipnn_res.frec_res.result));
	if (ret < 0) {

...
static void get_fddb_result_format(void *p, char *src_filename, char *dst_strbuf, int buf_len, int *data_len)
{
	...
	float im_w = (float)vipnn_res->input_param.img.width;
	float im_h = (float)vipnn_res->input_param.img.height;
	for (int i = 0; i < fd_res->obj_num; i++) {
#if 0
		float top_x = fd_res->result[i * 6 + 2] * im_w;
		float top_y = fd_res->result[i * 6 + 3] * im_h;
		float w = fd_res->result[i * 6 + 4] * im_w - top_x;
		float h = fd_res->result[i * 6 + 5] * im_h - top_y;
#else
		float top_x = fd_res->result[i * 6 + 2];
		float top_y = fd_res->result[i * 6 + 3];
		float w = fd_res->result[i * 6 + 4] - top_x;
		float h = fd_res->result[i * 6 + 5] - top_y;
#endif
		float score = fd_res->result[i * 6 + 1];
...
static void get_widerface_result_format(void *p, char *src_filename, char *dst_strbuf, int buf_len, int *data_len)
{
	...
	float im_ori_w = (float)vipnn_res->input_param.img.width;
	float im_ori_h = (float)vipnn_res->input_param.img.height;
	...
	for (int i = 0; i < fd_res->obj_num; i++) {
		...
		float top_x = (fd_res->result[i * 6 + 2] * TEST_IMG_WIDTH - x_off) / new_w * im_ori_w;
		float top_y = (fd_res->result[i * 6 + 3] * TEST_IMG_HEIGHT - y_off) / new_h * im_ori_h;
		float bottom_x = (fd_res->result[i * 6 + 4] * TEST_IMG_WIDTH - x_off) / new_w * im_ori_w;
		float bottom_y = (fd_res->result[i * 6 + 5] * TEST_IMG_HEIGHT - y_off) / new_h * im_ori_h;
...
static void nn_save_handler(char *file_name, uint32_t data_addr, uint32_t data_size)
{
	...
	memcpy(&pre_tensor_out, (void *)data_addr, data_size);
	...
	int data_len = 0;
#if TEST_DATA==WIDER_FACE_VAL
	get_widerface_result_format(&pre_tensor_out, file_name, dst_result_buf, sizeof(dst_result_buf), &data_len);
#elif TEST_DATA==FDDB_VAL
	get_fddb_result_format(&pre_tensor_out, file_name, dst_result_buf, sizeof(dst_result_buf), &data_len);
#endif

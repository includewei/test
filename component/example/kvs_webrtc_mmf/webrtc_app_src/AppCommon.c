...
static STATUS app_common_handleOffer(PAppConfiguration pAppConfiguration, PStreamingSession pStreamingSession, PSignalingMessage pSignalingMessage)
{
	...
	if (!mediaThreadStarted) {
		THREAD_CREATE_EX(&pAppConfiguration->mediaControlTid, APP_MEDIA_CONTROL_THREAD_NAME, APP_MEDIA_CONTROL_THREAD_SIZE, TRUE, app_common_runMediaSource,
						 (PVOID) pAppConfiguration);
	} else {
...
static STATUS app_common_onIceCandidatePairStats(UINT32 timerId, UINT64 currentTime, UINT64 userData)
{
	...
	CHK_WARN(MUTEX_WAITLOCK(pAppConfiguration->appConfigurationObjLock, 50 * HUNDREDS_OF_NANOS_IN_A_MILLISECOND) == TRUE, STATUS_APP_COMMON_ACQUIRE_MUTEX,
			 "app_common_onIceCandidatePairStats failed");
	locked = TRUE;
...
static STATUS app_common_pregenerateCertTimerCallback(UINT32 timerId, UINT64 currentTime, UINT64 userData)
{
	...
	CHK_WARN(MUTEX_WAITLOCK(pAppConfiguration->appConfigurationObjLock, 50 * HUNDREDS_OF_NANOS_IN_A_MILLISECOND) == TRUE, STATUS_APP_COMMON_ACQUIRE_MUTEX,
			 "app_common_pregenerateCertTimerCallback failed");
	locked = TRUE;
...
STATUS initApp(BOOL trickleIce, BOOL useTurn, PAppMediaSrc pAppMediaSrc, PAppConfiguration *ppAppConfiguration)
{
	...
	CHK(pAppMediaSrc != NULL, STATUS_APP_COMMON_NULL_ARG);
	CHK((pChannel = KVS_WEBRTC_CHANNEL_NAME) != NULL, STATUS_APP_COMMON_CHANNEL_NAME);
	...
	pAppSignaling->channelInfo.pRegion = KVS_WEBRTC_REGION == NULL ? DEFAULT_AWS_REGION : KVS_WEBRTC_REGION;
	...
	CHK_STATUS((app_signaling_init(pAppSignaling, app_common_onSignalingMessageReceived, app_common_onSignalingClientStateChanged,
								   app_common_onSignalingClientError,

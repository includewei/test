...
uint8_t bt_command_type(uint16_t command_type)
{
	...
	case BT_COMMAND_CHANGE_TO_PAIR_MODE:
...
void fATBP(void *arg)
{
	...
	if (!bt_command_type(BT_COMMAND_CHANGE_TO_PAIR_MODE)) {
		AT_PRINTK("[AT_PRINTK] ERROR: command type error!\n\r");
		goto exit;
	}

	bt_at_cmd_send_msg(BT_ATCMD_CHANGE_TO_PAIR_MODE, bt_at_cmd_buf);
	return;
...
void fATBt(void *arg)
{
	...
	param = atoi(argv[1]);
	if (param == 1) {
		AT_PRINTK("[ATBs]:_AT_BT_TAG_SCANNER_[ON]\n\r");
		bt_tag_scanner_app_init();
	} else if (param == 0) {
		AT_PRINTK("[ATBs]:_AT_BT_TAG_SCANNER_[OFF]\n\r");
...
#if defined(CONFIG_BT_A2DP_SINK) && CONFIG_BT_A2DP_SINK
extern void a2dp_app_main(void);
...
void fATB2(void *arg)
{
	...
	if (strcmp(argv[1], "sink") == 0) {
		if (strcmp(argv[2], "1") == 0) {
			AT_PRINTK("[ATB2]:a2dp_sink[ON]\n\r");
			a2dp_app_main();
		}
	}

exit:
	AT_PRINTK("[ATB2] Start A2DP Sink Profile: ATB2=sink, 1");
}
#endif
